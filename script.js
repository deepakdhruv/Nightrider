...
function fetchCatalogAsCSV() {
    return fetch('catalog.json')
        ...
        .then(catalogs => {
            ...
            // Convert catalog data to CSV format
            const searchTerm = document.getElementById("searchInput").value.toLowerCase();
...
function downloadCatalogCSV() {
    fetchCatalogAsCSV().then(encodedUri => {
...
document.getElementById("download-catalog-button").addEventListener("click", downloadCatalogCSV);
